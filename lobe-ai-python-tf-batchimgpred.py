...
if __name__ == "__main__":
    ...
    parser.add_argument("imagefolder", help="Path to your images folder.")
    parser.add_argument("imagetype", help="The type of imagefile you want to process.")
    args = parser.parse_args()

    imgfol = args.imagefolder
    ...
    # For opening the folder containing your images, and filter out everything that isn't the specified filetype
    listing = list(filter(lambda f: f.endswith(imgtype), os.listdir(imgfol)))
    ...
    for pics in listing:
        ...
        # Outputs a list with dictionaries
        labelDict = outputs['predictions']

        # WARNING ###
        # this script only looks for a model that uses two labels 
        # TODO: loop through all the labels that are used for this model
        NoOfLabels  = len(labelDict)

        labelName = labelDict[0]['label']
        labelScore = labelDict[0]['confidence']
        print("Label: " + labelName, end="\n")

        # create a list to save predicted labels and scores for each image 
        predList = [pics, im, labelName, labelScore]

        # append the information per image-prediction t the CSV
        createPredictionsCSV(predList)

        # copy image to created folder with predicted label 
        copyFilesToDestLabelFolders(im, labelName)